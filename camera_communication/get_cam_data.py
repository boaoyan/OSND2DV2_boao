...
from camera_communication.kalman_filter_points import KalmanFilterPoints
...
class UdpReceiverThread(QThread):
    ...
    ct_data_refreshed = pyqtSignal(object)
    pin_data_refreshed = pyqtSignal(object)
    ...
    def __init__(self, ip="127.0.0.1", port=8080, n_average=10):
        ...
        # 波形显示：缓存最近 100 帧
        self.waveform_max_len = 100
        self.raw_history = []  # 存原始平均值 (6,)
        self.filtered_history = []  # 存滤波后值 (6,)
    ...
    # @property
    # def pin_balls_in_cam(self):
    #     if self._pin_balls_in_cam_filtered is None:
    #         return None
    #     return self._pin_balls_in_cam_filtered if not self.pin_order_flipped else self._pin_balls_in_cam_filtered[::-1]
    ...
    #     if len(self.pin_buffer) >= 1:
    #         new_pin_avg = np.mean(self.pin_buffer, axis=0)  # shape (2, 3)
    #         # print("new_pin_avg_1.shape:", new_pin_avg_1.shape)
    #         # ✅ 对平均后的 pin 点进行 Kalman 滤波
    #         # new_pin_avg = self.filter_points.update(new_pin_avg_1)
    #         # self.data_ready.emit(new_pin_avg_1, new_pin_avg)
    #
    #         # print("new_pin_avg.shape (after filter):", new_pin_avg.shape)
    #         if self._last_pin_avg is None:
    #             self._pin_balls_in_cam = new_pin_avg
    #             self._last_pin_avg = new_pin_avg.copy()
    #             need_refresh = True
    ...
    #             dist = np.linalg.norm(new_pin_avg - self._last_pin_avg)
    #             if dist > 0.5:
    #                 self._pin_balls_in_cam = new_pin_avg.copy()
    #                 self._last_pin_avg = new_pin_avg.copy()
    #                 need_refresh = True
    #             # else: 不更新
    #
